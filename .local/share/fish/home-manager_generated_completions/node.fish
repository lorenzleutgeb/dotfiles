# node
# Autogenerated from man page /nix/store/50lhzpd68v47x6jy0b8b3p91n2zv3lv4-nodejs-22.14.0/share/man/man1/node.1.gz
...
complete -c node -l allow-addons -d 'Allow using native addons when using the permission model'
complete -c node -l allow-child-process -d 'Allow spawning process when using the permission model'
complete -c node -l allow-wasi -d 'Allow execution of WASI when using the permission model'
...
complete -c node -l disable-wasm-trap-handler -d 'Disable trap-handler-based WebAssembly bound checks and fall back to inline b…'
...
complete -c node -l entry-url -d 'Interpret the entry point as a URL'
...
complete -c node -l permission -d 'Enable the permission model'
...
complete -c node -l experimental-test-isolation -d 'Configures the type of test isolation used in the test runner'
complete -c node -l experimental-test-module-mocks -d 'Enable module mocking in the test runner'
complete -c node -l experimental-strip-types -d 'Enable experimental type-stripping for TypeScript files'
complete -c node -l experimental-transform-types -d 'Enable transformation of TypeScript-only syntax into JavaScript code'
complete -c node -l experimental-eventsource -d 'Enable experimental support for the EventSource Web API'
complete -c node -l no-experimental-fetch -d 'Disable experimental support for the Fetch API'
complete -c node -l no-experimental-websocket -d 'Disable experimental support for the WebSocket API'
complete -c node -l experimental-webstorage -d 'Enable experimental support for the Web Storage API'
...
complete -c node -l no-experimental-sqlite -d 'Disable the experimental node:sqlite module'
...
complete -c node -l inspect-wait -d 'Activate inspector on host:port and wait for debugger to be attached'
...
complete -c node -l localstorage-file -d 'The file used to store localStorage data'
...
complete -c node -l test-coverage-branches -d 'Require a minimum threshold for branch coverage (0 - 100)'
complete -c node -l test-coverage-exclude -d 'A glob pattern that excludes matching files from the coverage report'
complete -c node -l test-coverage-functions -d 'Require a minimum threshold for function coverage (0 - 100)'
complete -c node -l test-coverage-include -d 'A glob pattern that only includes matching files in the coverage report'
complete -c node -l test-coverage-lines -d 'Require a minimum threshold for line coverage (0 - 100)'
...
complete -c node -l test-update-snapshots -d 'Regenerates the snapshot file used by the test runner for snapshot testing'
