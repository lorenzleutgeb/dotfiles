# git
# Autogenerated from man page /nix/store/fxcgpjpzqlk44b70f4smyad6y6jnw8cw-git-with-svn-2.49.0/share/man/man1/git.1.gz
...
complete -c git -l no-lazy-fetch -d 'Do not fetch missing objects from the promisor remote on demand'
complete -c git -l no-optional-locks -d 'Do not perform optional operations that require locks'
complete -c git -l no-advice -d 'Disable all advice hints from being printed'
...
complete -c git -l type -d 'will convert to false'
complete -c git -s e -d '<object> to see if the object is locally available'
complete -c git -l object-format -d 'in git-init(1).  GIT_DEFAULT_REF_FORMAT'
complete -c git -l ref-format -d 'in git-init(1)'
