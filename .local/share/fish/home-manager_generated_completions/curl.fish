# curl
# Autogenerated from man page /nix/store/7jcschqa3fz84pmn46rd5ggvyhpgc9dd-curl-8.12.1-man/share/man/man1/curl.1.gz
complete -c curl -l variable -l variable -d 'set to a single dash (-))'
...
complete -c curl -s s -l silent -d 'VERSION This man page describes curl 8. 12. 1'
...
complete -c curl -l ntlm -l digest -l negotiate
...
complete -c curl -l ciphers -d '(TLS) Specifies which cipher suites to use in the connection if it negotiates…'
...
complete -c curl -s b -l cookie -d '(HTTP) This option has two slightly separate cookie sending functions'
...
complete -c curl -l output-dir -d 'directories it mentions already exist, no directories are created'
...
complete -c curl -l data-raw -d 'the @ character.  To post data purely binary, you should instead use the'
complete -c curl -l data-binary
complete -c curl -l data-urlencode
...
complete -c curl -l dump-ca-embed -d '(TLS) Write the CA bundle embedded in curl to standard output, then quit'
complete -c curl -s D -l dump-header -d '(HTTP FTP) Write the received protocol headers to the specified file'
complete -c curl -l next
...
complete -c curl -l header -d '(HTTP IMAP SMTP) Extra header to include in information sent'
complete -c curl -s h -l help -d 'Usage help.  Provide help for the subject given as an optional argument'
...
complete -c curl -l 'http0.9' -d '(HTTP) Accept an HTTP version 0. 9 response'
...
complete -c curl -l no-keepalive
...
complete -c curl -s i -l show-headers
complete -c curl -l location-trusted
...
complete -c curl -l max-filesize -d '(FTP HTTP MQTT) When set to a non-zero value, it specifies the maximum size (…'
...
complete -c curl -s v -l verbose -l trace -l trace-ascii
...
complete -c curl -l no-clobber -d 'When used in conjunction with the -o, --output, -J, --remote-header-name,'
complete -c curl -l remote-name-all -d 'that already exist'
complete -c curl -l clobber -s J -l remote-header-name -d specified
...
complete -c curl -l url -s u -l user
...
complete -c curl -l proxy-basic -d 'Use HTTP Basic authentication when communicating with the given proxy.  Use'
...
complete -c curl -l proxy-ciphers -d '(TLS) Same as --ciphers but used in HTTPS proxy context'
complete -c curl -l proxy-crlfile -d 'Provide filename for a PEM formatted file with a Certificate Revocation List …'
complete -c curl -l proxy-digest -d 'Use HTTP Digest authentication when communicating with the given proxy.  Use'
...
complete -c curl -l proxy-tls13-ciphers -d '(TLS) Same as --tls13-ciphers but used in HTTPS proxy context'
complete -c curl -l proxy-tlsauthtype -d 'Set TLS authentication type with HTTPS proxy'
complete -c curl -l proxy-tlspassword -d 'Set password to use with the TLS authentication method specified with'
complete -c curl -l proxy-tlsuser -d set
...
complete -c curl -o 500 -d 'specifies the last 500 bytes 9500- specifies the bytes from offset 9500 and f…'
...
complete -c curl -l retry-delay
complete -c curl -l retry-max-time
...
complete -c curl -l skip-existing -d 'If there is a local file present when a download is requested, the operation …'
...
complete -c curl -l ssl-sessions -d '(TLS) Use the given file to load SSL session tickets into curl\'s cache before…'
...
complete -c curl -s t -l telnet-option -d 'Pass options to the telnet protocol.  Supported options are:'
...
complete -c curl -l tls-earlydata -d '(TLS) Enable the use of TLSv1'
...
complete -c curl -l tlsuser
complete -c curl -s 1 -l tlsv1 -d '(TLS) Use at least TLS version 1. x when negotiating with a remote TLS server'
...
complete -c curl -l trace-ids -l trace-time
...
complete -c curl -l no-verbose -o vv -d 'means that "-vv -v" is equivalent to a single -v'
complete -c curl -l vlan-priority -d '(All) Set VLAN priority as defined in IEEE 802. 1Q'
complete -c curl -s w -l write-out -d '3 If --write-out is provided several times, the last set value is used'
