# htop
# Autogenerated from man page /nix/store/ga9xf6ml3bs8ccl3pky83yb6i7j4x9x9-htop-3.4.1/share/man/man1/htop.1.gz
complete -c htop -s d -l delay -d 'Delay between updates, in tenths of a second'
complete -c htop -s C -l no-color -l no-colour -d 'Start htop in monochrome mode'
complete -c htop -s F -l filter -d 'Filter processes by terms matching the commands'
complete -c htop -s h -l help -d 'Display a help message and exit'
complete -c htop -s p -l pid -d 'Show only the given PIDs'
complete -c htop -s s -l sort-key -d 'Sort by this column (use --sort-key help for a column list)'
complete -c htop -s u -l user -d 'Show only the processes of a given user, or self if omitted'
complete -c htop -s U -l no-unicode -d 'Do not use unicode but ASCII characters for graph meters'
complete -c htop -s M -l no-mouse -d 'Disable support of mouse control'
complete -c htop -l readonly -d 'Disable all system and process changing features'
complete -c htop -s V -l version -d 'Output version information and exit'
complete -c htop -s t -l tree -d 'Show processes in tree view'
complete -c htop -s H -l highlight-changes -d 'Highlight new and old processes'
complete -c htop -l drop-capabilities -d 'Linux only; this option needs to have been enabled at compile-time and requir…'

