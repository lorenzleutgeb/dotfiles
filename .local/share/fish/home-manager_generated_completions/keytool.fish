# keytool
# Autogenerated from man page /nix/store/5yii18di89wpxcffk3cihpzfdrzw54gl-openjdk-21.0.7+6/share/man/man1/keytool.1
...
complete -c keytool -o showinfo
complete -c keytool -o version -d 'COMMANDS FOR CREATING OR ADDING DATA TO THE KEYSTORE'
...
complete -c keytool -o alias -d 'The passphrase may be supplied via the standard input stream; otherwise the u…'
...
complete -c keytool -o destalias -d 'If a destination alias is not provided, then the command prompts you for one'
...
complete -c keytool -o keysize -d '    2048 (when using -genkeypair and -keyalg is [dq]DSA[dq])     3072 (when u…'
...
complete -c keytool -o srcalias -d 'password -destkeypass password'
