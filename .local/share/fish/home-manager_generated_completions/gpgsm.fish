# gpgsm
# Autogenerated from man page /nix/store/wfsqj2kiqsnrp24gjfrw2bprrmzca31i-gnupg-2.4.7/share/man/man1/gpgsm.1.gz
...
complete -c gpgsm -l log-time -d 'Prefix all log output with a timestamp even if no log file is used. P    '
...
complete -c gpgsm -l input-size-hint -d 'This option can be used to tell GPGSM the size of the input data in bytes'
...
complete -c gpgsm -l no-pretty-dn -d 'By default gpgsm prints distinguished names (DNs) like the Issuer or Subject …'
...
complete -c gpgsm -l chuid -d 'Change the current user to uid which may either be a number or a name'
...
complete -c gpgsm -l debug -d 'Set debug flags.   All flags are or-ed and flags may be given in C syntax (e'
...
complete -c gpgsm -l show-certs -d 'This command takes certificate files as input and prints information about th…'
...
complete -c gpgsm -l status-fd -d '``total\'\' if that is not available by other means'
