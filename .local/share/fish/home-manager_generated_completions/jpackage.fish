# jpackage
# Autogenerated from man page /nix/store/5yii18di89wpxcffk3cihpzfdrzw54gl-openjdk-21.0.7+6/share/man/man1/jpackage.1
complete -c jpackage -l type -s t -d 'The type of package to create Valid values are: {[dq]app-image[dq], [dq]exe[d…'
...
complete -c jpackage -l dest -s d -d 'Path where generated output file is placed (absolute path or relative to the …'
...
complete -c jpackage -l add-modules -d 'A comma ([dq],[dq]) separated list of modules to add This module list, along …'
...
complete -c jpackage -l no-man-pages -l no-header-files
...
complete -c jpackage -l mac-sign -d 'Request that the package or the predefined application image be signed'
...
complete -c jpackage -l launcher-as-service -d 'Request to create an installer that will register the main application launch…'
...
complete -c jpackage -l linux-rpm-license-type -d 'Type of the license ([dq]License: value[dq] of the RPM . spec)'
complete -c jpackage -l linux-app-release -d 'Release value of the RPM <name>'
complete -c jpackage -l linux-app-category -d 'Group value of the RPM /. spec file or Section value of DEB control file'
