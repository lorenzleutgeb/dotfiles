...
NIX_DIRENV_FALLBACK_NIX=${NIX_DIRENV_FALLBACK_NIX:-/nix/store/1b068wbq8d6fpii8mv0kknhqmq556xp9-nix-2.28.3/bin/nix}
...
_require_version() {
  ...
  if ! printf "%s\n" "$required" "$version" | LC_ALL=C /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/sort -c -V 2>/dev/null; then
    _nix_direnv_error \
      "minimum required $(/nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/basename "$cmd") version is $required (installed: $version)"
    return 1
  fi
}
...
_nix_direnv_preflight() {
  ...
  if [[ ! -d "$layout_dir/bin" ]]; then
    /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/mkdir -p "$layout_dir/bin"
  fi
  ...
  /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/cat >"${layout_dir}/bin/nix-direnv-reload" <<-EOF
#!/usr/bin/env bash
set -e
if [[ ! -d "$PWD" ]]; then
...
EOF
  ...
  if [[ ! -x "${layout_dir}/bin/nix-direnv-reload" ]]; then
    /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/chmod +x "${layout_dir}/bin/nix-direnv-reload"
  fi
...
_nix_import_env() {
  ...
  if [[ -n ${NIX_BUILD_TOP+x} && $NIX_BUILD_TOP == */nix-shell.* && -d $NIX_BUILD_TOP ]]; then
    /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/rm -rf "$NIX_BUILD_TOP"
  fi
...
_nix_clean_old_gcroots() {
  ...
  /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/rm -rf "$layout_dir/flake-inputs/"
  /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/rm -f "$layout_dir"/{nix,flake}-profile*
}
...
_nix_argsum_suffix() {
  ...
  if [ -n "$1" ]; then
    ...
    if has sha1sum; then
      out=$(/nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/sha1sum <<<"$1")
  ...
  if [[ ! -e $profile ||
    ...
    if [[ $_nix_direnv_manual_reload -eq 1 && -z ${_nix_direnv_force_reload-} ]]; then
      ...
      if tmp_profile_rc=$(_nix print-dev-env --profile "$tmp_profile" "$@"); then
        ...
        /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/rm -f "$tmp_profile" "$tmp_profile"*

        # also add garbage collection root for source
        local flake_input_paths
        /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/mkdir -p "$flake_inputs"
  ...
  if path=$(_nix eval --impure --expr "<nixpkgs>" 2>/dev/null); then
    if [[ -f "${path}/.version-suffix" ]]; then
      ...
      read -r version_prefix < <(
        /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/cat "${path}/.version"
        echo
      )
...
# resholve: keep /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/basename
# resholve: keep /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/cat
# resholve: keep /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/chmod
# resholve: keep /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/mkdir
# resholve: keep /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/rm
# resholve: keep /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/sha1sum
# resholve: keep /nix/store/cg09nslw3w6afyynjw484b86d47ic1cb-coreutils-9.7/bin/sort
